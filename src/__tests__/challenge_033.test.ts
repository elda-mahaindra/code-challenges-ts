import type * as Challenge033 from "../challenges/challenge_033";

const { ErrorEnum, solution } = jest.requireActual<typeof Challenge033>(
  "../challenges/challenge_033.ts"
);
...
const successCases = [
  ...
  },
  {
    ...
    input: {
      n: 3,
      assignments: ["A[0..0] = 69", "B[-3..-3] = 1547", "C[133..133] = 55"],
      element: "B[-3]",
    },
    output: 1547,
  },
...
const failureCases = [
  ...
  },
  {
    ...
    input: {
      n: 3,
      assignments: ["A[-1..1] = 1 2 3", "B[3..7] = 3 4 5 6 7"],
      element: "A[0]",
    },
    error: ErrorEnum.INVALID_ARRAY_ASSIGNMENTS,
  },
  {
    id: 2,
    input: {
  ...
  },
  {
    ...
    error: ErrorEnum.INVALID_ELEMENT,
  },
  {
    id: 4,
    input: {
...
describe("test challenge 033", () => {
  it.each(successCases)("success case $id", ({ input, output }) => {
    const { n, assignments, element } = input;

    expect(solution(n, assignments, element)).toBe(output);
  });

  it.each(failureCases)("failure case $id", ({ input, error }) => {
    const { n, assignments, element } = input;

    expect(() => solution(n, assignments, element)).toThrow(error);
  });
});
